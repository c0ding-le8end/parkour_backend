...
from models import User, Parking_history
import uuid
...
def login():
    ...
    if not auth or not auth.get('email') or not auth.get('password'):
        response = make_response(jsonify({'status': 'Enter valid credentials','auth':auth,'email':auth.get('email'),'password':auth.get('password')}),
